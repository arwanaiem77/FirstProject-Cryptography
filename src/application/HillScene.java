...
public class HillScene extends Pane {	
	HillCipher obj =new HillCipher();
	String keyHill;
	String Plaintext;
	String Ciphertext;
	public HillScene(Stage primaryStage,Scene oldScene) {
...
//			keyHill=obj.seed();
		...
		encrypt.setOnAction(a ->{
			if(keyHill!=null) {
				ctTF.setText(obj.encrypt(Plaintext, keyHill));
				Ciphertext=obj.encrypt(Plaintext, keyHill);
			}
		...
		decrypt.setOnAction(a ->{
			res.setText(obj.decrypt(Ciphertext, keyHill));
		});
		fileP.setOnAction(a ->{
